...
try:
    ft_rowid = int(os.path.basename(options.csfile))
    print ft_rowid
except ValueError as e:
    print "ERROR: Invalid CS filepath"
    sys.exit(-1)
...
try:
    ...
    for [clt_rowid, clt_inode, clt_count, clt_chunkRowIDs] in db.execute('SELECT clt_rowid,clt_inode,clt_count,clt_chunkRowIDs FROM CSStorageChunkListTable'):
        ...
        for i in range(0,len(str(clt_chunkRowIDs))/8):
            ...
            # Extracting chunks
            for [offset, dataLen, cid, ft_rowid_] in db.execute("SELECT offset,dataLen,cid,ft_rowid from CSChunkTable where ct_rowid = '%s'" % chunk_id):

                if ft_rowid != ft_rowid_:
                    print "Chunk is not in specified storage file"
                    break
